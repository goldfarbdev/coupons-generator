...
foreach ($lines as $line_num => $line) {
	...
	if(preg_match('/Valid in U.S. stores only/',$coupon[0])){
		...
		if($coupon[9] != "" && trim($coupon[10]) == ""){
			$couponcode = 'In-Store/Online/Phone Coupon Code: <span class="z_coupon_code_bold">' . $coupon[9] . '</span>';
		}elseif($coupon[9] != "" && trim($coupon[10]) != ""){
			$couponcode = 'In-store Coupon Code: <span class="z_coupon_code_bold">' . $coupon[9] . '</span><br />Online or Phone Coupon Code: <span class="z_coupon_code_bold">' . $coupon[10] . '</span>';
		}
		$printpot = '<a href="#" class="z_print_this z_print_this' . $t . '">Print</a>';
		$shoppot = '<a href="' . $coupon[13] . '" class="z_get_offer z_noprint">Shop now</a>';
	}elseif(preg_match('/Not valid in stores/',$coupon[0])){
		$couponcode = 'Online or Phone Coupon Code: <span class="z_coupon_code_bold">' . $coupon[10] . '</span>';
		$printpot = '';
		$shoppot = '<a href="' . $coupon[13] . '" class="z_get_offer z_noprint">Shop now</a>';
		$barcodeimage = '';
	}
